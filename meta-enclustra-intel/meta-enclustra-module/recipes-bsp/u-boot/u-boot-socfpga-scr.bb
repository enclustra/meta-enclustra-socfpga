...
#	mkimage -A arm -O linux -T script -C none -a 0 -e 0 -n "QSPI Script" -d "${WORKDIR}/u-boot-qspi.txt" ${WORKDIR}/boot-qspi.scr
	mkimage -A arm -O linux -T script -C none -a 0 -e 0 -n "SD Card Script" -d "${WORKDIR}/u-boot-sd.txt" ${WORKDIR}/u-boot-sd.scr
        cp ${WORKDIR}/u-boot-sd.scr ${WORKDIR}/u-boot.scr
        cp ${WORKDIR}/u-boot-sd.scr ${WORKDIR}/boot.scr
}
